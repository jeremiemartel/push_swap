...
/*   Updated: 2019/04/27 13:03:24 by jmartel          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
{
	if (!*argv)
		return (ck_error(*push, "No arguments given"));
	if (ck_parsing_fill_push(push, argv) == FAILURE)
		return (FAILURE);
	if (ck_parsing_check_duplication(*push) == FAILURE)
		return (ck_error(*push, "Duplication detected"));
	return (SUCCESS);
}

/*
** ck_parsing_fill_push:
...
int			ck_parsing_fill_push(t_push **push, char **argv)
{
	...
	while (argv[i])
	{
		if (!(tab = ft_split_whitespaces(argv[i])))
			return (ck_error(*push, "Internal Error"));
		j = 0;
		while (tab[j])
		{
			if (ck_parsing_is_number(tab[j]) == FAILURE)
				return (ck_error(*push, "Bad format detected"));
			if (ck_parsing_push_list(push, tab[j]) == FAILURE)
				return (ck_error(*push, "Number is too big"));
			j++;
		}
		(*push)->lena += j;
		ft_strtab_free(tab);
